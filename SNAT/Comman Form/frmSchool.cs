...
namespace SNAT.Comman_Form
{
    public partial class frmSchool : Form
    {
        ...
        void FillDepartment()
        {
            try
            {
                string strSqlQuery = "SELECT  id,code,name,status,remarks FROM SNAT.dbo.M_School sc ORDER BY sc.code";
                    ...
                    if (ClsMessage.showAskDeleteMessage() == DialogResult.Yes)
                    {
                        ...
                        if (dtSchool != null && dtSchool.DefaultView.Count > 0)
                        {
                            if (dtSchool.GetChanges() != null)
                            {
                                ...
                                bool lReturn = false;
                                strSqlQuery = "SELECT  id,code,name,status,remarks FROM SNAT.dbo.M_School sc where 1=2";
        ...
        private bool ValidateSave()
        {
            try
            {
                ...
                {
                    errorProvider1.SetError(txtCode, "School code cannot be left blank.");
                    return false;
                }
                ...
                {
                    errorProvider1.SetError(txtName, "School name cannot be left blank.");
                    return false;
                }
                if (ClsUtility.FormMode == ClsUtility.enmFormMode.AddMode)
                {
            ...
            try
            {
                ...
                if (dtSchool != null && dtSchool.DefaultView.Count > 0)
                {
                    ...
                    if (dtSchool.GetChanges() != null)
                    {
                        ...
                        bool lReturn = false;
                        strSqlQuery = "SELECT  id,code,name,status,remarks FROM SNAT.dbo.M_School where 1=2";
                        lReturn = ClsDataLayer.UpdateDataAdapter(strSqlQuery, dtSchool);
